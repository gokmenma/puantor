...
// G: Geldi (1), X: Gelmedi (2), İ: İzinli (3), R: Raporlu (4) vb.
$types = [
...
<div class="container px-0">
        ...
        <div class="d-flex align-items-center justify-content-between mb-2">
            ...
            <div class="d-flex gap-2">
                ...
                <input type="date" class="form-control form-control-sm w-auto border-0 bg-secondary-lt" 
                       ...
                       onchange="location.href='?p=puantaj&date='+this.value">
            </div>
        </div>
    ...
    <div class="list-group list-group-mobile mb-5">
        <?php foreach ($persons as $person): 
            // Mevcut puantajı çek
            // $current_status = $puantajModel->getStatus($person->id, $selected_date);
...
function savePuantaj(personId, typeId, typeCode, color) {
    ...
    // UI Güncelleme (Hızlı hissettirmesi için hemen yapıyoruz)
    ...
    // Gerçek Kayıt İşlemi (AJAX)
    // Bu kısım ana projedeki puantaj kaydetme endpoint'ine yönlendirilmelidir.
    console.log(`Kaydediliyor: Personel ${personId}, Tarih ${date}, Tür ${typeId}`);
    
    /* 
    $.post('../api/puantaj_save.php', {
        person_id: personId,
        date: date,
        type_id: typeId
    }, function(response) {
