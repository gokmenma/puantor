...
");
    ...
    */
    ...
    public function deletePersonMonthlyRecords($person_id, $month, $year)
    {
        // Maas, gelir ve kesintileri sil
        $sql1 = $this->db->prepare("DELETE FROM maas_gelir_kesinti WHERE person_id = :person_id AND ay = :month AND yil = :year");
        $sql1->execute([':person_id' => $person_id, ':month' => $month, ':year' => $year]);

        // Puantaj tablosundaki tutarlari sifirla (Mavi Yaka icin)
        $firstDay = Date::firstDay($month, $year);
        $lastDay = Date::lastDay($month, $year);
        $sql2 = $this->db->prepare("UPDATE puantaj SET tutar = 0 WHERE person = :person_id AND gun >= :start_date AND gun <= :end_date");
        $sql2->execute([':person_id' => $person_id, ':start_date' => $firstDay, ':end_date' => $lastDay]);
